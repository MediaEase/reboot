document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', function (event) {
        ...
        if (button) {
            const cardTitle = button.id.replace('_options', '');
            const menuId = `${cardTitle}_options_menu`;
...
export function generateOptionsMenu(appName) {
    return `
        <div class="hidden absolute right-0 mt-2 w-48 bg-white divide-y divide-gray-100 rounded-md shadow-lg z-50" id="${appName}_options_menu">
